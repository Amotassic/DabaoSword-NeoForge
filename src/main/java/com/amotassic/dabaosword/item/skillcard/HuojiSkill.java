    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        if (!entity.level().isClientSide && entity instanceof Player player && noTieji(entity)) {
            ...
            if (cd == 0 && !stack1.isEmpty() && stack1.is(Tags.BASIC_CARD)) {
                setCD(stack, 15);
                viewAs(player, Tags.BASIC_CARD, ModItems.FIRE_ATTACK.get(), Sounds.HUOJI.get());
            }
        }
        super.curioTick(slotContext, stack);
    }

    public static void viewAs(@NotNull Player player, TagKey<Item> tag, Item item, SoundEvent sound) {
