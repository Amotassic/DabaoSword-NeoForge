...
public class Wu {
    ...
    public static class Buqu extends SkillItem {
        ...
        public int onDying(LivingEntity user, LivingEntity target, Skill skill, ExData data) {
            if (user instanceof Player player && player.isDeadOrDying()) {
                ...
                var card = c(newCard());
                player.level().players().forEach(p -> p.displayClientMessage(Component.translatable("buqu.tip1", player.getDisplayName(), skill.toHoverableText(), card.toStack().getDisplayName(), card.rank.rank), false));
                if (chuang.isEmpty()) {
                    ...
                    if (!chuang.contains(card.rank.rank)) {
                        ...
                        if (chuang.split(", ").length < 3) player.displayClientMessage(Component.translatable("buqu.tip2").withStyle(RED), false);
                    }
                }
                skill.setNbt(nbt);
            }
    ...
    public static class Fanjian extends SkillItem implements DabaoSwordCommand.CSkill {
        ...
        public boolean activeSkill(Player user, Skill skill) {
            if (skill.getCD() == 0 && countCards(user) > 0 && !user.getTags().contains("seen_skill_tip")) {
        ...
        public boolean activeSkill(Player user, Skill skill, LivingEntity target) {
            ...
            if (target instanceof Player player && countCards(user) > 0) { //有手牌才能发动
                ...
                player.displayClientMessage(Component.translatable("select_a_suit", fanjianText(user, player, Suit.Heart), fanjianText(user, player, Suit.Diamond), fanjianText(user, player, Suit.Spade), fanjianText(user, player, Suit.Club)), false);
                player.displayClientMessage(Component.translatable("fanjian.time.tip").withStyle(BOLD), false);
                return true;
            }
            return false;
        }
        ...
        public void triggerSkill(LivingEntity entity, Skill skill, LivingEntity target, int value) {
            if (entity instanceof Player user) {
                ...
                if (target instanceof Player tp) tp.displayClientMessage(message, false);
                ...
                if (c.suit.ordinal() + 1 != value) {
                    ...
                    target.hurt(getDamageSource(user, DamageTypes.MAGIC), 6);
                }
                skill.setCD(20);
            }
        }
    ...
    public static class Kurou extends SkillItem {
        ...
        public boolean activeSkill(Player user, Skill skill) {
            if (user.getHealth() + 5 * countCard(user, canSaveDying) > 4.99) {
                ...
                voice(user, this);
                return true;
            } else {user.displayClientMessage(Component.translatable("item.dabaosword.kurou.tip").withStyle(RED), true);}
            return false;
        }
    }
    ...
    public static class Zhijian extends SkillItem {
        ...
        public boolean activeSkill(Player user, Skill skill, LivingEntity target) {
            ...
            if (isEquipment.test(itemStack)) {
                ...
                draw(user);
                return true;
            } else user.displayClientMessage(Component.translatable("zhijian.fail").withStyle(RED), true);
            return false;
        }
    }
}
