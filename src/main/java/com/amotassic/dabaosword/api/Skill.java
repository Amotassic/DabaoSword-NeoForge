...
public interface Skill {
     ...
     * @return tuple的左边填增伤倍率，右边填固定数值的加减伤害量。增伤倍率可以为负值，结算方式如下：
     * <p>
     * 伤害结算公式：最终伤害 = [原始伤害 x (1 + 增伤倍率和) + 固定数值加减伤] x (1 + 负增伤倍率)。
     * （由此可知：你不可能在完全相同的条件下返回既增伤，又最终减伤，这是我想到最简洁的仅利用两个值就能完成伤害结算的办法）
     */
    default Tuple<Float, Float> modifyDamage(LivingEntity target, DamageSource source, float amount) {return null;}
    ...
    default void onClickGUISlot(Player player, ItemStack stack, Player target, ItemStack selected, int slotIndex) {}
    ...
    default int onDrawPhase(Player player, ItemStack stack) {return 0;}
}
